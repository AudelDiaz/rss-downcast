...
def test_v2_schema_created_cleanly_without_legacy_archive(mod, tmp_path):
    """v2: fresh schema has feeds+episodes with feed_id; no legacy migration."""
    db = str(tmp_path / 'fresh.db')
    conn = mod.setup_database(db)
    tables = {r[0] for r in conn.execute("SELECT name FROM sqlite_master WHERE type='table'")}
    assert 'episodes' in tables
    assert 'feeds' in tables
    assert 'episodes_old_pre_multi_feed' not in tables
    cols = [r[1] for r in conn.execute('PRAGMA table_info(episodes)')]
    assert 'feed_id' in cols
    conn.close()

    # Idempotent on second call: same tables, no error
